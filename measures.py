...
class CrosslingualRetrieval(abc.ABC):
    ...
    def mrr_score(self):
        ...
        for doc_index in range(total_docs):
            ...
            #print('query_doc_distrib',query_doc_distrib.T)
            #print('self.doc_topics2',self.doc_topics2.T)
...
class TopicDiversity(Measure):
    ...
    def score(self, topk=10):
        ...
        if topk > len(self.topics):
            ...
            td_en = len(unique_words) / (topk * len(self.topics))
        print('diversity',td_en)    
        return td_en
...
class CoherenceNPMI():
    def __init__(self, topics,texts_en,topk=5, measure='c_npmi'):
        """
        Initialize metric
        ...
        self.texts_en = []
        self.texts_en=texts
        self.dictionary_en = Dictionary(texts_en)
        self.topics1=topics
        self.topk = topk
    ...
    def score(self, model_output=None, top_words=None):
        ...
        if self.topk > len(self.topics1):
            raise Exception('Words in topics are less than topk')
        else:
            print(len(self.topics1))
            #print(self.texts_en)
            sum=0
            for k in range(len(self.topics1)):
               npmi_en = CoherenceModel(topics=self.topics1[k], texts=self.texts_en, dictionary=self.dictionary_en,
                                  coherence=self.measure, processes=1, topn=self.topk)
               a=npmi_en.get_coherence()
               if a=='nan':
                  sum=sum
               else:
                  sum=sum+a
            sum=sum/len(self.topics1)
            print('npmi',sum)
            return npmi_en.get_coherence()
