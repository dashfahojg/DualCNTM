...
# decooder network
from networks.decoding_bow import TrilingualDecoderNetwork
...
class TrilingualContrastiveTM:
    ...
    def __init__(self,bow_size, contextual_size, pretrain_en1,pretrain_cn,pretrain_en2,pretrain_ja,vocab_en1,vocab_cn,
                 ...
                 num_data_loader_workers=1, label_size=0, loss_weights=None, languages=None):
    ...
    def export_beta_en(self,beta, vocab):
        ...
        file_utils.save_text(topic_str_list, path=f'/home/ssliang/M3L-topic-model/output/ECNews_en_7--topic=40.txt')
    ...
    def export_beta_cn(self,beta, vocab):
        ...
        num_top_word = 15
        topic_str_list = file_utils.print_topic_words(beta, vocab, num_top_word=15)
        file_utils.save_text(topic_str_list, path=f'/home/ssliang/M3L-topic-model/output/ECNews_cn_7--topic=40.txt')
        return topic_str_list
    def export_beta_ja(self,beta, vocab):
        beta=beta.cpu().detach().numpy()
        num_top_word = 15
        topic_str_list = file_utils.print_topic_words(beta, vocab, num_top_word=15)
        file_utils.save_text(topic_str_list, path=f'/home/ssliang/M3L-topic-model/output/ECNews_ja_7--topic=40.txt')
        return topic_str_list
    ...
    def _train_epoch(self, loader):
        ...
        for batch_num, batch_samples in enumerate(loader):
            ...
            theta2_1, theta3_1new, theta2_1w_new, theta3w_new= self.model(X_bow1,X_bow2)
            ...
            #co_encn=torch.matmul(beta[1].T,beta[0])
            #co_enja=torch.matmul(beta[1].T,beta[2])
            #co_encn/=co_encn.sum()
            #co_enja/=co_enja.sum()
            #sim1=F.binary_cross_entropy_with_logits(co_encn.float(),M_encn.float())
            #sim2=F.binary_cross_entropy_with_logits(co_enja.float(),M_enja.float())
            #betaloss=sim1+sim2
            infoNCE_cross = self._infoNCE_loss(theta1,theta2_new)
            infoNCE_cross += self._infoNCE_loss(theta3w_new, theta2_1w_new)
            infoNCE_cross += self._infoNCE_loss(theta2_1,theta3_1new)
            infoNCE_cross += self._infoNCE_loss(theta2w, theta1w_new)
            rl_loss=rl_loss1+rl_loss2+rl_loss3+rl_loss4
            sim = self.MSEloss(theta1,theta1w_new)
            sim += self.MSEloss(theta3_1new,theta3w_new)
            #loss = 0.04*rl_loss.sum() + 1000*infoNCE_cross+0.1*kl_cross.sum()
            loss =10*kl_cross.mean()+infoNCE_cross*6+13*rl_loss.mean()
            print('rl',rl_loss.mean(),'kl',kl_cross.mean(),'info',infoNCE_cross)
            list = [loss,rl_loss.mean(),kl_cross.mean(),infoNCE_cross]
    
            data = pd.DataFrame([list])
            data.to_csv('./output/kl2_record_betaloss1,5_kl10——topic==10.csv', mode='a', header=False, index=False) 
    ...
    def fit(self, train_dataset, validation_dataset, save_dir=None, verbose=True, patience=5, delta=0):
        ...
        pbar.close()
        return theta1,theta2_new,theta2_1,theta3_1new,self.model.beta
    ...
    def _validation(self, loader):
        ...
        return samples_processed, val_loss,theta1,theta2_new,theta2_1,theta3_1new
